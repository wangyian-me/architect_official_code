...
1. global constraint:
    1) edge: at the edge of the room, close to the wall.
    2) middle, horizontal/vertical: not close to the edge of the room, and its direction.
    3) corner: at the corner of the room

2. soft location constraint:
    1) the predicted bounding-box location of the object.
...
4. position constraint:
    ...
    3) behind, object: behind of another object object.
    4) left of, object: to the left of another object.
    5) right of, object: to the right of another object.

5. alignment constraint:
    1) center aligned, object: align the center of the object with the center of another object.

6. rotation constraint:
    1) face to, object: face to the center of another object.



Your job is to add the Rotation constraint to some objects if reasonable.
For example, for those objects and constraints: 
sofa-0 | edge | location, [1.79, 1.59, 0.39]
coffee table-0 | middle, horizontal | location, [2.01, 2.01, 0.21] | near, sofa-0 | in front of, sofa-0 | center aligned, sofa-0
you can add rotation constraint to the coffee table and output the following:
sofa-0 | edge | location, [1.79, 1.59, 0.39]
coffee table-0 | middle, horizontal | location, [2.01, 2.01, 0.21] | near, sofa-0 | in front of, sofa-0 | center aligned, sofa-0 | face to, sofa-0

Currently, we have the following constraints:
{constraints}

Note that the latter objects could only depend on the former objects. Plus, there are several objects that are already placed in the scene. They can also be depended on.
Currently, these are the objects already placed in the scene:
{placed_obj}

Please follow the desired format *strictly* (do not add any additional text at the beginning or end) to provide the additional rotation constraints if reasonable."""
...
The output format must be: wall object | location, [x, y, z] (| above, floor object )| distance from floor (cm).
...
Your job is to add a reasonable distance from floor constraint to objects.
For example, for those objects and constraints: 
painting-0 | location, [1.33, 0.08, 3.3] | above, sofa-0
switch-0 | location, [0.35, 2.21, 4.11]
you can add rotation constraint to them and output the following:
painting-0 | location, [1.33, 0.08, 3.3] | above, sofa-0 | 160
switch-0 | location, [0.35, 2.21, 4.11] | 120
